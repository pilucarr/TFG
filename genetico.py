...
# -*- coding: utf-8 -*-
"""
Created on Thu Jun 20 13:23:48 2024
...
class genetic:
    def __init__(self, Ie, params, real_data, ind_train, ngen, pop_size, rmut, p_bounds, p_ini, p_size=59, crossover_rate = 0.2):
        self.pop = []
        self.off = [] #offspring
        self.V0 = real_data[0,:]
        self.Ie = Ie
    ...
    def _createindv(self): 
        fitness = 1000000000000000000000000000000000000000
        while fitness == 1000000000000000000000000000000000000000:
            indv = []
            for i in range(self.p_size):
                bounds = self.p_bounds[i]
                a,b = bounds
                indv.append(np.random.uniform(a,b))
            
            fitness = self._fitness_function(indv)
            
        return {"Individual": indv, "Fitness": fitness}
        
    
    def _fitness_function(self, indv):
        try:
            tot_error = 0
            for ind in self.ind_train:
                Vrec, out_range = simulate_model(indv, self.Ie[ind-1], self.V0[ind], self.params)
                if out_range == 1:
                    tot_error = 1000000000000000000000000000000000000000
                elif len(Vrec)<3200:
                    tot_error = 1000000000000000000000000000000000000000
                else:
                    error = np.mean((Vrec - self.real_data[:,ind] )**2)
                    #dist_peak = abs( max(Vrec)-max(self.real_data[:,ind]))
    ...
    def _createpop(self):
        ...
        if self.p_ini != None:
            p_ini_fit = self._fitness_function(self.p_ini)
            self.pop = [{"Individual": self.p_ini, "Fitness": p_ini_fit}]
        l = len(self.pop)
        for _ in range(self.pop_size - l):
            self.pop.append(self._createindv())
        return self.pop
    ...
    def _evolve(self): 
        ...
        # Change here depending on how many individuals you want to keep for your evolutions
        self.pop = self.pop[:self.pop_size//2]
        while len(self.pop) < self.pop_size:
            if random.random() < self.crossover_rate and len(self.pop) < self.pop_size-1:
                parent_a, parent_b = random.choices(self.pop, k=2)
                child1, child2 = self._cross(parent_a["Individual"], parent_b["Individual"])
                self.pop.extend([child1, child2])
            else:
                parent = random.choice(self.pop)
                mutated_parent = self._mutation(parent["Individual"])
                self.pop.append(mutated_parent)
                    
    
    def _cross(self, parent_a, parent_b):
        ...
        fitness_a = self._fitness_function(child_a)
        fitness_b = self._fitness_function(child_b)
        
        return {"Individual": child_a, "Fitness": fitness_a},{"Individual": child_b, "Fitness": fitness_b}
        
    
    def _mutation(self, indv):
        
        if random.random() < self.rmut:
            index = random.randint(0, self.p_size - 1)
            bounds = self.p_bounds[index]
            a,b = bounds
            newvalue = random.uniform(a, b)
            indv[index] = newvalue
        
        fitness_mut = self._fitness_function(indv)
        
        return {"Individual": indv, "Fitness": fitness_mut}
        
        
    def run(self):
        self._createpop()
        for n in range(self.ngen):
            # Evolve the population during x generations 500 by default
            self._evolve()
            best = sorted(self.pop, key=lambda x: x["Fitness"], reverse=False)[0]
            print("en la generación ", n)
            print("El mejor es: ", best)
...
p_ini = [0, 15.5, -5, 12.08, 0, 35.2, 72.7, 16.7, 0, 0.38685, 122.35, 15.29,
...
genetic_obj = genetic(I_ext, params, Tot_datos_control, ind_train, 50, 10, 0.05, p_bounds, p_ini=p_ini)

best = genetic_obj.run()
print("El mejor del genético es")
print (best)


Vmodel, out_range = simulate_model(best["Individual"], Ie, V0, params)
